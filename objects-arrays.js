...
function roar(tyran) {
  console.log(`${tyran} "RAWERSRARARWERSARARARRRR!"`);
}
roar(dinosaur1["objectname"]);
...
const universities = graduates.map(univer => {
  return univer.university;
});
...
graduates.forEach(function(contactInfo) {
  console.log(`${contactInfo.first_name}, ${contactInfo.email}`);
});

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
let uni = [];
const validstring = ["Uni"];
for (let i = 0; i < graduates.length; i++) {
  if (graduates[i].university.includes(validstring)) {
...
zooAnimals.forEach(function(animalNames) {
  console.log(
    `Name: ${animalNames.animal_name} Scientific: ${
      animalNames.scientific_name
    }`
  );
});
...
*/
let populationTotal = zooAnimals.reduce((total, populate) => {
  return (total += populate.population);
}, 0);
